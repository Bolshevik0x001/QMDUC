...
class MultiChannelSequentialCircuit(nn.Module):
    ...
    def __init__(self, num_qubits: int, in_channels: int, layers_per_channel: int = 1):
        ...
        self.weights_per_layer = num_qubits * 3
...
class DP_QMDUC_Conv2d(nn.Module):
...
class DP_QMDUC(nn.Module):
    ...
    def __init__(self):
        ...
        # 超参数配置
        KERNEL_SIZE = 2
        LAYERS_PER_CHANNEL = 1
        NUM_KERNELS = 1

        self.quantum_conv = DP_QMDUC_Conv2d(
            in_channels=3,
...
def save_history_to_shared_csv(history, model_name, filename="cifar_DP_history.csv"):
    data = {
        'model': model_name,
...
def train_model(model_name="CIFAR_DP_result(1k1d)"):

    train_loader, test_loader = load_cifar10_data()

    model = DP_QMDUC().to(device)
