...
def load_cifar10_data(root="./data", batch_size=128,
    ...
    test_loader = DataLoader(test_dataset,
                             batch_size=test_samples_per_class * 10,
...
class ChannelWiseQuantumCircuit(nn.Module):
    ...
    def __init__(self, in_channels: int, kernel_size: int, layers_per_position: int = 1):
        ...
        Args:
            in_channels: 输入通道数（量子比特数）
            kernel_size: 卷积核尺寸
            layers_per_position: 每个位置编码后的参数化电路层数
        """
        super().__init__()

        self.in_channels = in_channels  # 量子比特数 = 通道数
        self.kernel_size = kernel_size
        self.num_positions = kernel_size * kernel_size  # 总位置数
        self.layers_per_position = layers_per_position

        # 计算每层参数化电路的权重数量
        self.weights_per_layer = in_channels * 3  # 每个量子比特3个旋转门

        # 计算总权重数量：每个位置后有layers_per_position层参数化电路
...
class ChannelWiseQuantumKernel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            output: [batch_size, num_kernels * in_channels, out_height, out_width]
        """
        batch_size, channels, height, width = x.shape
...
class ChannelWiseQuantumConv2d(nn.Module):
    """
    通道优先量子卷积层
    """
    ...
    def __init__(
            ...
            layers_per_position: int = 1,  # 每个位置编码后的参数化电路层数
...
class ChannelWiseQCNN(nn.Module):
    ...
    def __init__(self):
        ...
        # 超参数配置
        KERNEL_SIZE = 2
        LAYERS_PER_POSITION = 1  # 每个位置编码后应用1层参数化电路
        NUM_KERNELS = 1 # 2个并行的量子卷积核

        # 通道优先量子卷积层
        self.quantum_conv = ChannelWiseQuantumConv2d(
            in_channels=3,  # RGB三通道 = 3个量子比特
...
def train_model(model_name="channel_QMDC(1k1d)_31"):
    """训练模型并记录训练过程中的指标"""
    # 数据加载
    train_loader, test_loader = load_cifar10_data()

    # 模型初始化
    model = ChannelWiseQCNN().to(device)
